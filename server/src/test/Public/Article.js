...
// const { createDefaultPost, getPostSchema, createTags } = require('../Helpers');
...
// 			res.body.should.have.property('pages').eql(Math.ceil(expectedLength / maxResponse));
...
// 		});
// 		it('it should GET the posts by tag name that are approved', async () => {
// 			/* artistic tag is found in tags */
// 			const tag = createdTags.find(t => t.name === 'technology');
// 			/* beforeEach hook is creating 1000 posts alternating 3 realms with 5 different status types */
// 			const res = await chai.request(server)
// 				.get(`/articles/tags/${ tag._id }`);
// 			res.body.should.be.a('object');
// 			res.body.should.have.property('results');
// 			res.body.results.forEach(async post => {
// 				const saved = await Post.findById(post._id);
// 				expect(saved.status).to.equal(ReviewStatus.approved);
// 			});
// 		});
// 	});
// });
