...
export default {
   ...
   * at that index with payload
	...
	[types.UPDATE_ARTICLE](state, payload) {
		const index = state.articles.findIndex((article) => article._id === payload.article._id);
		if (index !== -1) {
	...
	[types.DELETE_ARTICLE](state, payload) {
		const count = payload.deleteCount.n;
		if (!count) {
			return;
		}
		const index = state.articles.findIndex((article) => article._id === payload.id);
	...
	[types.EDIT_TAG_VAL](state, payload) {
		...
		const tag = state.tags.find((tag) => tag._id === payload.id);
